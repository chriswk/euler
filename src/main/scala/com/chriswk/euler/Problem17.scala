...
object Problem17 extends App {
  ...
  lazy val name: Int => Int = {
    ...
    case n if (n < 1000) => name(n / 100) + 7 + (if (n % 100 > 0) 3 + name(n % 100) else 0)
    case 1000 => 11
  }

  val answer = Profiling.timed(Profiling.printTime("Numbers to letters in ")) { (1 to 1000).map(name).sum }
  print(answer)
}