...
/** * Euler published the remarkable quadratic formula: n² + n + 41 It turns out
  ...
  * 39. However, when n = 40, 402 + 40 + 41 = 40(40 + 1) + 41 is divisible by
  ...
  * the coefficients, 79 and 1601, is 126479. Considering quadratics of the
  * form: n² + an + b, where |a| 1000 and |b| 1000 where |n| is the
...
object Problem27 extends App {
  val values = (-999 until 1000).flatMap { a =>
    (-999 until 1000).map(b =>
      (a, b, (0 to 1000).view.takeWhile(n => isPrime(n * n + a * n + b)).size)
    )
  }
