...
object Problem23 extends App {
  val findSumOfFactors =
    eagerSumOfDivisors(28123).zipWithIndex.filter(p => p._1 > p._2).map(_._2)
  val integersAsAbundant = findSumOfFactors.flatMap { a =>
    findSumOfFactors.takeWhile(_ <= (28123 - a)).map(b => a + b)
  }
